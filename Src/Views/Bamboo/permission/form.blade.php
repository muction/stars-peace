...
@section('car-body')
    <form class="form-horizontal" method="post" action="{{ route( 'rotate.permission.storage' ,['infoId'=> isset($info['id']) ? $info['id'] : null ] ) }}">
        @csrf
        ...
        <div class="form-group  @error('title') has-error @enderror">
            <label for="title" class="col-sm-2 control-label">权限类型<span class="required">*</span></label>
            <div class="col-sm-10">

                <select class="form-control" name="type" style="width: 300px">
                    @foreach($types as $ty)
                        <option value="{{ $ty['id'] }}" @if( isset($info['type']) && $info['type'] == $ty['id'] ) selected="selected" @endif >
                            {{ $ty['title'] }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="form-group  @error('title') has-error @enderror">
            <label for="title" class="col-sm-2 control-label">权限名称 <span class="required">*</span></label>
            <div class="col-sm-10">
